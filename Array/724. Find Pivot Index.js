/*
Given an array of integers nums,
write a method that returns the "pivot" index of this array.

We define the pivot index as the index where the sum of the numbers to the left of the index is equal to the sum of the numbers to the right of the index.

If no such index exists, we should return -1.
If there are multiple pivot indexes, you should return the left-most pivot index.

Example 1:
Input:
nums = [1, 7, 3, 6, 5, 6]
Output: 3
Explanation:
The sum of the numbers to the left of index 3 (nums[3] = 6) is equal to the sum of numbers to the right of index 3.
Also, 3 is the first index where this occurs.

Example 2:
Input:
nums = [1, 2, 3]
Output: -1
Explanation:
There is no index that satisfies the conditions in the problem statement.

Note:
1. The length of nums will be in the range [0, 10000].
2. Each element nums[i] will be an integer in the range [-1000, 1000].
 */

/*
题目：
假设有一个整数数组，写一个方法，返回这个数组的“枢轴”索引。
如果一个索引左边所有元素的和等于这个索引右边所有元素的和，我们就把这个索引定义为枢轴索引。
如果这个索引不存在，我们就返回-1.
如果有多个枢轴索引，就返回最靠近左边的那一个枢轴索引。

分析：
首先，遍历数组，将数组中所有数的和保存在sum变量中，
然后再遍历数组，进行判断，
如果存在枢轴索引，
那么。枢轴索引对应的数加上它左边所有数的和leftSum的两倍，
一定等于数组中所有数的和sum。
 */

/**
 * @param {number[]} nums
 * @return {number} index
 */
var pivotIndex = function(nums) {
    var index = -1,
        sum = 0,
        leftSum = 0;
    for (var i=0; i<nums.length; i++) {
        sum += nums[i];
    }
    for (var j=0; j<nums.length; j++) {
        if (nums[j] === sum - leftSum * 2) {
            index = j;
            break;
        }
        leftSum += nums[j];
    }
    return index;
};
